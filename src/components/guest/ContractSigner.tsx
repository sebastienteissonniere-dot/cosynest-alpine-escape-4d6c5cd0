...
export const ContractSigner: React.FC<ContractSignerProps> = ({ reservation, onSigned }) => {
  ...
  const draw = (e: React.MouseEvent<HTMLCanvasElement> | React.TouchEvent<HTMLCanvasElement>) => {
    ...
    ctx.strokeStyle = '#9B6B43'; // Warm alpine brown stroke
    ctx.lineWidth = 2.5;
    ctx.lineCap = 'round';
    ...
    return (
      <Card className="bg-emerald-50/80 border-emerald-200 text-emerald-950 rounded-2xl shadow-sm">
        <CardHeader className="flex flex-row items-center gap-3 pb-2">
          ...
          <div>
            <CardTitle className="text-base font-serif font-bold text-emerald-900">
  ...
  return (
    <Card className="bg-white border-amber-900/10 text-amber-950 rounded-2xl shadow-sm">
      <CardHeader className="pb-3 border-b border-amber-900/10">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-2">
            <FileText className="w-5 h-5 text-[#9B6B43]" />
            <CardTitle className="text-base font-serif font-bold text-amber-950">Contrat de Location Saisonnière Directe</CardTitle>
          </div>
      ...
      <CardContent className="space-y-4 pt-4">
        {/* Termes du contrat */}
        <div className="bg-amber-50/50 p-3.5 rounded-xl border border-amber-900/10 text-xs text-amber-900/80 max-h-48 overflow-y-auto space-y-2 leading-relaxed">
          <p className="font-bold text-[#9B6B43] font-serif">CONTRAT DE LOCATION DE VACANCES - CHALET COSYNEST</p>
          <p>
          ...
          <p className="font-bold text-amber-950">1. Usage du bien :</p>
          <p>
          ...
          <p className="font-bold text-amber-950">3. Règlement intérieur :</p>
          <p>
        ...
        <div className="flex items-start space-x-3 pt-1">
          <Checkbox
            ...
            className="mt-0.5 border-amber-900/30 data-[state=checked]:bg-[#9B6B43] data-[state=checked]:border-[#9B6B43]"
          />
          <label htmlFor="terms" className="text-xs text-amber-950 font-medium leading-snug cursor-pointer">
        ...
        <div className="space-y-2">
          <div className="flex justify-between items-center text-xs text-amber-900/70 font-medium">
            <span className="flex items-center gap-1"><Edit3 className="w-3.5 h-3.5 text-[#9B6B43]" /> Signer dans l'encadré :</span>
            {hasSignature && (
              <button onClick={clearCanvas} className="text-rose-700 hover:text-rose-800 flex items-center gap-1 text-[11px] font-semibold">
                <Trash2 className="w-3 h-3" /> Effacer
              </button>
            )}
          </div>
          <div className="border border-amber-900/15 rounded-xl overflow-hidden bg-amber-50/30">
            <canvas
              ref={canvasRef}
              width={340}
              height={120}
